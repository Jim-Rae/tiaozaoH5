...
$.extend($, {
    compressPic: function (options,callback) {
        var para = {
            limitSize: 2,
            multiple: true,
        ...
        var inputstr = '<input id="fileImage"  style="display:none;" accept="image/*"  type="file"' + multiple + '>';
        $self.before(inputstr);
        ...
        var core = {
            fileSelected: function () {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    //     return;
                    // }
                    if (item.size > 1024 * 1024 * para.limitSize) {
                        console.log("图片大于"+para.limitSize+"M，开始进行压缩...");

                        (function(img) {
                            var mpImg = new MegaPixImage(img);
                            var resImg = document.createElement("img");
                            resImg.file = img;
                            mpImg.render(resImg, { maxWidth: 400, maxHeight: 400, quality: 0.7 }, function() {
                                // var base64 = getBase64Image(resImg);
                                var base64 = resImg.src;
                                console.log("base64", base64.length, simpleSize(base64.length));
        ...
        $(para.filebutton).click(function () {
            $("#fileImage").click();
        });
